...
public class iface1Sar extends AllGraph {
    ...
    public iface1Sar(kSar hissar, String s1) {
        ...
        this.ifName = s1;
        t_rxpck = new TimeSeries("ipacket/s", org.jfree.data.time.Second.class);
        mysar.dispo.put("IF " + s1 + " ipacket/s", t_rxpck); 
        t_txpck = new TimeSeries("opacket/s", org.jfree.data.time.Second.class);
        mysar.dispo.put("IF " + s1 + " opacket/s", t_txpck);
        t_rxbyt = new TimeSeries("ibits/s", org.jfree.data.time.Second.class);
        mysar.dispo.put("IF " + s1 + " ibits/s", t_rxbyt);
        t_txbyt = new TimeSeries("obits/s", org.jfree.data.time.Second.class);
        mysar.dispo.put("IF " + s1 + " obits/s", t_txbyt);
        t_ierr = new TimeSeries("ierror/s", org.jfree.data.time.Second.class);
        mysar.dispo.put("IF " + s1 + " ierror/s", t_ierr);
        t_oerr = new TimeSeries("oerror/s", org.jfree.data.time.Second.class);
        mysar.dispo.put("IF " + s1 + " oerror/s", t_oerr);
        t_util = new TimeSeries("%util", org.jfree.data.time.Second.class);
        mysar.dispo.put("IF " + s1 + " %util", t_util);
    }
    ...
    public void add(Second now, Float val1, Float val2, Float val3, Float val4, Float val5, Float val6, Float val7) {
        ...
        this.t_ierr.add(now, val5, do_notify());
        this.t_oerr.add(now, val6, do_notify());
        this.t_util.add(now, val7, do_notify());
        number_of_sample++;
    }
    ...
    public void addtotree(DefaultMutableTreeNode myroot) {
        mynode = new DefaultMutableTreeNode(new GraphDescription(this, "ESARIFACE1", this.Title, null));
    ...
    public JFreeChart makegraph(Second g_start, Second g_end) {
        ...
        XYPlot subplot1 = new XYPlot(xydataset1, null, new NumberAxis("ipacket/opacket /s"), minichart1);
        // rws
        ...
        XYPlot subplot2 = new XYPlot(bytset, null, new NumberAxis("ibits/obits /s"), minichart2);
        //
        ...
        XYPlot subplot3 = new XYPlot(cmpset, null, new NumberAxis("ierr/oerr /s"), minichart3);
        //
        ...
        XYPlot subplot4 = new XYPlot(mcstset, null, new NumberAxis("%util"), minichart4);
        //
