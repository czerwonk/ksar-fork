...
public class loadSar extends AllGraph {
    ...
    public loadSar(kSar hissar) {
        ...
        t_nproc = new TimeSeries("nproc", org.jfree.data.time.Second.class);
        mysar.dispo.put("nproc", t_nproc);
    }
    ...
    public void add(Second now, Float val1, Float val2, Float val3, Float val4) {        
        ...
        this.t_nproc.add(now, val4, do_notify());        
        number_of_sample++;
    }

    
    public XYDataset createnproc() {
        TimeSeriesCollection graphcollection = new TimeSeriesCollection();
        graphcollection.addSeries(this.t_nproc);
    ...
    public void addtotree(DefaultMutableTreeNode myroot) {
        mynode= new DefaultMutableTreeNode(new GraphDescription(this, "ESARLOAD", this.Title, null));
        mysar.add2tree(myroot,mynode);
    }

    public JFreeChart makegraph(Second g_start, Second g_end) {
        // used
        XYDataset xydataset1 = this.createnproc();
        XYItemRenderer minichart1 = new StandardXYItemRenderer();
        minichart1.setBaseStroke(kSarConfig.DEFAULT_STROKE);
        minichart1.setSeriesPaint(0, kSarConfig.color1);
        XYPlot subplot1 = new XYPlot(xydataset1, null, new NumberAxis("nproc"), minichart1);        
        //
