 ...
 * @author alex
 */
public class JpgExport implements Runnable {
    ...
    public void run() {
        ...
        if (dohtml) {
            try {
                ...
                System.err.println("Unable to open file: " + basefilename + "_index.html");
                out = null;
            }
        }
        if (out != null) {
        ...
        for (Iterator<String> it = tmphash.keySet().iterator(); it.hasNext();) {
            ...
            String name = (new File(basefilename + "_" + crackfilename(key + ".jpg"))).getName();
            value.saveJPG(mysar.startofgraph, mysar.endofgraph, basefilename + "_" + crackfilename(key + ".jpg"), width, height);
            if (out != null) {
                try {
