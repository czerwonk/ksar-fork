 ...
 * @author alex
 */
public class kmalgSar extends AllGraph {

    public kmalgSar(final kSar hissar) {
        super(hissar);
        Title = "Kernel large page";
        t_mem = new TimeSeries("Reserved", org.jfree.data.time.Second.class);
        mysar.dispo.put("Kernel large page Reserved", t_mem);
        t_alloc = new TimeSeries("Allocated", org.jfree.data.time.Second.class);
        mysar.dispo.put("Kernel large page Allocated", t_alloc);
        t_failed = new TimeSeries("Failed", org.jfree.data.time.Second.class);
        mysar.dispo.put("Kernel large page Failed", t_failed);
        // Collection
    ...
    public void addtotree(final DefaultMutableTreeNode myroot) {
        mynode = new DefaultMutableTreeNode(new GraphDescription(this, "SOLARISKMALG", this.Title, null));
