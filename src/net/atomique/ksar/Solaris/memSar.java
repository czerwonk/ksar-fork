...
public class memSar extends AllGraph {
    ...
    public void add(final Second now,final  Float memInit,final  Float swapInit) {
        this.t_freemem.add(now, memInit, do_notify()); // remove because of bug *8192);
        this.t_freeswap.add(now, swapInit.floatValue() * 512, do_notify());
        number_of_sample++;
    }

    public void addtotree(final DefaultMutableTreeNode myroot) {
        mynode = new DefaultMutableTreeNode(new GraphDescription(this, "SOLARISMEM", this.Title, null));
    ...
    public JFreeChart makegraph(final Second g_start, final Second g_end) {
        ...
        if (mysar.solarispagesize == -1) {
            decimalformat1 = new IEEE1541Number(0);
        } else {
            decimalformat1 = new IEEE1541Number(mysar.solarispagesize);
        }
